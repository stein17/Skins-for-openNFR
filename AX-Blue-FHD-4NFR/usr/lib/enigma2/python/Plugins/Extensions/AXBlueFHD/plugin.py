...
config.plugins.AXBlue = ConfigSubsection()
config.plugins.AXBlue.refreshInterval = ConfigNumber(default=10)
config.plugins.AXBlue.woeid = ConfigNumber(default = 638242)
config.plugins.AXBlue.tempUnit = ConfigSelection(default="Celsius", choices = [
                                ("Celsius", _("Celsius")),
                                ("Fahrenheit", _("Fahrenheit"))
                                ])

def Plugins(**kwargs):
    return [PluginDescriptor(name=_("AXBlue FHD Configtool"), description=_("Personalize your AXBlue FHD (Skin by stein17)"), where = [PluginDescriptor.WHERE_PLUGINMENU],
    icon="plugin.png", fnc=main)]

def main(session, **kwargs):
    if config.skin.primary_skin.value == "AX_Blue_FHD_4NFR/skin.xml":
        session.open(AXBlue_Config)
    else:
        AddPopup(_('Please activate AXBlue FHD Skin before run the Config Plugin'), type=MessageBox.TYPE_ERROR, timeout=10)
        return []
...
class AXBlue_Config(Screen, ConfigListScreen):

    skin = """
            <screen name="AXBlue_Config" position="center,center" size="1280,720" title="AXBlue Setup" >
                    <widget source="Title" render="Label" position="70,47" size="950,43" font="Regular;35" transparent="1" />
    ...
    def keyYellow(self):
        if self.myAtileHD_active.value:
            self.session.openWithCallback(self.AXBlueScreenCB, AXBlueScreens)
        else:
    ...
    def select_weather_id_callback(self, res):
        if res and isInteger(res):
            print(res)
            config.plugins.AXBlue.woeid.value = int(res)
    ...
    def about(self):
        self.session.open(AXBlue_About)
...
class AXBlueScreens(Screen):

    skin = """
            <screen name="AXBlueScreens" position="center,center" size="1280,720" title="AXBlue Setup">
                    <widget source="Title" render="Label" position="70,47" size="950,43" font="Regular;35" transparent="1" />
